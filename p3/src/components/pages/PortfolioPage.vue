<template>
	<div id="portfolio-page" class="portfolio">
		<h2 align="left">
			Portfolio Holdings - Current Market Value ${{ format(totalValue, 2) }}
		</h2>
		<button v-if="modeEditConfirm" v-on:click="editConfirmPressed">
			{{ modeEditConfirm }}
		</button>
		<table class="portfolioTable">
			<tr>
				<th v-for="(heading, column) in tableHeadings" v-bind:key="column">
					{{ heading }}
				</th>
			</tr>
			<tr v-for="(holding, index) in holdings" v-bind:key="index">
				...
				<td>
					<div v-if="companiesPriceList">
						<label
							v-for="(marketPrice, index2) in companiesPriceList"
							v-bind:key="index2"
						>
				...
				<td>
					<div v-if="companiesPriceList">
						<label
							v-for="(marketPrice, index3) in companiesPriceList"
							v-bind:key="index3"
						>
							<label v-if="marketPrice.symbol == holding.symbol">
				...
				<td>
					<div v-if="companiesPriceList">
						<label
							v-for="(marketPrice, index4) in companiesPriceList"
							v-bind:key="index4"
						>
							<label v-if="marketPrice.symbol == holding.symbol">
								{{
		...
		<button v-if="modeAddConfirm" v-on:click="addConfirmPressed">
			{{ modeAddConfirm }}
		</button>
		...
		methods: {
			...
			deleteCancelPressed: function(symbol) {
				if (this.modeAddConfirm == "Confirm") {
					...
					this.modeAddConfirm = "Add";
				} else if (this.modeAddConfirm == "Add") {
					...
					app.axios
						...
						.then(response => {
							if (this.holdings.length > 1) {
								...
								this.companiesPriceList[0] = response.data;
			...
			format: function(num, digits) {
				...
				return (
					(num / si[i].value).toFixed(digits).replace(rx, "$1") + si[i].char
				);
			}
		},
		mounted() {
			...
			if (this.holdings.length > 0 && !this.symbolToAdd) {
				this.getMarketPrice();
			} else if (this.symbolToAdd) {
