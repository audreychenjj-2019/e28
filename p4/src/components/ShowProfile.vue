			...
			<tr>
				...
				<td>
					...
					<p>
						...
						<span
							v-if="indexComponent"
						>Index Component: {{indexComponent}}</span>
					</p>
...
export default {
	...
	mounted() {
		if (
			this.symbolId == this.lastResearchedSymbol &&
			this.lastResearchedSymbolProfile
		) {
			this.companyProfile = this.lastResearchedSymbolProfile;
		} else {
			app.axios
				.get(app.config.baseProfileUrl + this.symbolId)
				.then(response => {
					this.companyProfile = response.data.profile;
					this.$store.commit(
						"setLastResearchedSymbol",
						this.symbolId
					);
					this.$store.commit(
						"setLastResearchedSymbolProfile",
						this.companyProfile
					);
				}),
				error => {
		...
		app.axios
			.get(app.config.baseIndexComponentUrl + this.symbol)
			.then(response => {
